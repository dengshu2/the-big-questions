...
## 八、上线后迭代记录（2026-07-04 ~ 07-05）

1. **天球完整可见**：slice → meet，宽屏不再裁切星座；星座标注加 Q 编号/星数/书数副标；开场动效（座形线描绘、星点渐显、星尘闪烁，respects reduced-motion）
2. **L1 时间轴退场**：星座页改为「天球该角落的放大图」——同一套星位保持座形连续，对话弧线画在星间，年代降级为星名下小字；移动端纵向星图册保留。语义缩放的隐喻自此贯通
3. **三层内容体系**：星志 54 篇（一等星 47 人全覆盖）+ 简志 125 篇（二等星全覆盖，Sub-Agent 并行生成 + 事实纪律约束 + 低把握名单抽查）+ 星签 420 条全覆盖，存于 `data/blurbs.json`（line / brief 字段），构建期校验
4. **标签矩形避让**：星名按文本框宽度（随字数）避让，放大图画布按包围盒撑满、高度随人数增长；全部星图文字加纸色描边光晕。已验证最密的 Q8 零残余叠压
5. **源数据修正**（内容生成过程中发现）：泰戈尔 1851-1964→1861-1941、德·沙泽尔生年 1904→1940、安迪·克拉克国籍→英国

## 九、待办与后续方向

- [ ] AB 连线彩蛋：任选两颗星沿对话关系找路径并高亮（数据已就绪，纯前端 BFS）
- [ ] 对话关系扩充（现 91 条，每星座 10~20 条的密度可继续加）
- [ ] 三四等星按需补简志（Sub-Agent 流程可复用，prompt 模板见 git 历史）
- [ ] 恒星页书目跨问题重复条目（如《论语》在 Q0/Q10 各一条）的去重展示策略
- [ ] 放大图座形与可读性的松紧参数（NEED_Y、hw）可按口味微调
